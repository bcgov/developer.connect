...
class ApiGateway:
    ...
    def create_key(cls, org_id: int, request_json: Dict[str, str]):
        ...
        if not cls._consumer_exists(
            email, env
        ):  # If the account doesn't have api access, add it
    ...
    def _get_api_gw_key(cls, env):
        ...
        return (
            current_app.config.get("API_GW_KEY")
            if env == "prod"
            else current_app.config.get("API_GW_NON_PROD_KEY")
        )
    ...
    def _create_consumer(cls, name, org_details, env):
        ...
        client_rep = generate_client_representation(
            org_details.get("id"),
        ...
        KeycloakService.add_user_to_group(
            service_account.get("id"), GROUP_ACCOUNT_HOLDERS
        )
        org_name = cls._make_string_compatible(org_details.get("name"))
        branch_name = cls._make_string_compatible(
            org_details.get("branchName") or "BCR"
        )
    ...
    def _filter_and_add_keys(cls, api_keys_response, keys, email):
        def _add_key_to_response(_key):
            if _key["keyStatus"] == "approved":
                _key["email"] = email
                _key["environment"] = (
                    "prod" if _key["environment"] == "prod" else "sandbox"
                )
    ...
    @classmethod
    @user_context
    def _create_payment_account(cls, org_details: dict, **kwargs):
        ...
        if not current_app.config.get("PAY_API_SANDBOX_URL"):
            current_app.logger.warning(
                "Sandbox URL not provided, skipping sandbox pay account creation"
            )
            return
        user: UserContext = kwargs["user_context"]
        pay_account = cls._get_pay_account(org_details.get("id"), user)
        pay_request = {
            "accountId": org_details.get("id"),
            "accountName": f"{org_details.get('name')}-{org_details.get('branchName', '')}",
            "padTosAcceptedBy": pay_account.get("padTosAcceptedBy", ""),
            "bcolAccountNumber": org_details.get("bcolAccountId", ""),
            "bcolUserId": org_details.get("bcolUserId", ""),
            "paymentInfo": {
                "methodOfPayment": pay_account.get("futurePaymentMethod", None)
                or pay_account["paymentMethod"],
    ...
    def _create_sandbox_pay_account(cls, pay_request, user):
        ...
        pay_sandbox_accounts_endpoint = (
            f"{current_app.config.get('PAY_API_SANDBOX_URL')}/accounts?sandbox=true"
        )
        RestService.post(
            endpoint=pay_sandbox_accounts_endpoint,
    ...
    @classmethod
    def _get_pay_account(cls, org_id, user):
        pay_accounts_endpoint = (
            f"{current_app.config.get('PAY_API_URL')}/accounts/{org_id}"
        )
        pay_account = RestService.get(
            endpoint=pay_accounts_endpoint, token=user.bearer_token
        ).json()
        return pay_account

    @classmethod
    @user_context
    def _get_org_details(cls, org_id, **kwargs):
        user: UserContext = kwargs["user_context"]
        org_endpoint = f"{current_app.config.get('AUTH_API_URL')}/orgs/{org_id}"
        org_details = RestService.get(
            endpoint=org_endpoint, token=user.bearer_token
        ).json()
        return org_details
